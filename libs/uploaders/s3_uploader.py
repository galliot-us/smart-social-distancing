...
class S3Uploader:
  ...
  def upload_file(self, bucket, file_path, file_name, bucket_path):
    ...
    try:
      response = self.s3_resource.Bucket(bucket).upload_file(Filename=f"{file_path}/{file_name}", Key=f"{bucket_path}/{object_name}")
  ...
  def upload_cv_image(self, bucket, cv_image, file_name, bucket_path):
    ...
    try:
      response = self.s3_resource.Bucket(bucket).put_object(Body=img, Key=f"{bucket_path}/{object_name}")
    except ClientError as e:
      self.logger.error(f'Error when uploading: {e}')
      return False
    return True
